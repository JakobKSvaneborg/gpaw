...
def initialize_G_maps(symmetries: QSymmetries, qpd: SingleQPWDescriptor):
    ...
    # Precompute reverse lookup Q_G value -> position once; each symmetry
    # then reduces to O(nG) dict lookups instead of O(nG) linear searches
    # per G-vector (i.e. O(nG^2) total per symmetry).
    Q_to_idx = {int(q): i for i, q in enumerate(Q_G)}
    ...
    for U_cc, sign, shift_c in symmetries:
        ...
        try:
            G_G = np.fromiter((Q_to_idx[int(uq)] for uq in UQ_G),
