...
class Chi0DysonEquations:
    ...
    def calculate_vchi_symm(self, direction='x', modified=False):
        ...
        # In-place broadcast multiplication over all frequencies at once.
        vchi0_symm_wGG *= sqrtv_G  # last-axis scaling
        vchi0_symm_wGG *= sqrtv_G[:, np.newaxis]  # second-to-last-axis scaling
...
class CustomizableDielectricFunction(DielectricFunctionData):
    ...
    def macroscopic_customized_dielectric_function(self):
        ...
        # We only need the [0, 0] element of the inverse; solving a single
        # RHS is ~O(nG^3/3) vs full inversion ~O(nG^3) and avoids a large
        # temporary matrix.
        eps_w = np.zeros((self.wblocks.nlocal,), complex)
        e0 = np.zeros(self.eps_wGG.shape[-1], dtype=complex)
        e0[0] = 1.0
