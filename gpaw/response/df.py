...
class CustomizableDielectricFunction(DielectricFunctionData):
    ...
    def macroscopic_customized_dielectric_function(self):
        ...
        if self.wblocks.nlocal > 0:
            # Instead of computing a full inverse just to read the (0, 0)
            # element (O(nG^3)), solve for the first column of the inverse:
            # eps_GG @ x = e_0 gives x[0] = inv(eps_GG)[0, 0]. Same asymptotic
            # cost, but avoids materializing the full (nG, nG) inverse.
