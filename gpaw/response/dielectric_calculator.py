...
class _DielectricFunctionCalculator:
    ...
    def eps_GG_gwp(self):
        gwp_M_GG = self.I_GG - self._chiVVfxc_GG() + self.chiVV_GG
        return self.I_GG - np.linalg.solve(gwp_M_GG, self.chiVV_GG)

    def eps_GG_gws(self):
        # Note how the signs are different wrt. gwp.
        # Nobody knows why.
        gws_M_GG = self.I_GG + self._chiVVfxc_GG() - self.chiVV_GG
        return np.linalg.solve(gws_M_GG, self.I_GG - self.chiVV_GG)
