...
def get_inverted_pw_mapping(qpd1, qpd2):
    ...
    # Build a hash-lookup once so the pairing is O(nG) instead of the
    # nested-loop O(nG**2) version.
    lookup = {tuple(int(x) for x in G2_c): G2
              for G2, G2_c in enumerate(G2_Gc)}
    try:
        mG2_G1 = [lookup[tuple(int(-x) for x in G1_c)] for G1_c in G1_Gc]
    except KeyError as err:
        raise ValueError('Could not match qpd1 and qpd2') from err
