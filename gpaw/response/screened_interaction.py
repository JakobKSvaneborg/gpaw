...
class WCalculator(WBaseCalculator):
    ...
    def calculate_W_wGG(self, chi0, fxc_mode='GW',
        ...
        # Loop-invariant outer product; also precompute the diagonal update
        # applied below in the q0 branch.
        ...
        for iw, (einv_GG, W_GG) in enumerate(zip(einv_wGG, W_wGG)):
            ...
            np.multiply(einvt_GG, sqrtVV_GG, out=W_GG)
    ...
    def dyson_and_W_new(self, iq, q_c, chi0, ecut, coulomb):
        ...
        # sqrtV and its outer product are frequency-independent; hoist them.
        sqrtV_G = coulomb.sqrtV(chi0.qpd, q_v=None)
        sqrtVV_GG = sqrtV_G * sqrtV_G[:, np.newaxis]
        I_GG = np.eye(nG)
        for iw, chi0_GG in enumerate(chi0.chi0_wGG):
            e_GG = I_GG - chi0_GG * sqrtVV_GG
        ...
        # Precompute loop-invariant identity slice and outer-product slice.
        I_gG = np.identity(nG)[my_gslice]
