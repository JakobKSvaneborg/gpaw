...
def calculate_bootstrap_kernel(qpd, chi0_GG, context):
    ...
    for iscf in range(120):
        ...
        # solve(A, B) computes inv(A) @ B without materializing the inverse
        # (same asymptotic cost, ~3x less work + more stable).
        chi_GG = np.linalg.solve(
            np.eye(nG, nG) - np.dot(chi0_GG, Kxc_GG), chi0_GG)
