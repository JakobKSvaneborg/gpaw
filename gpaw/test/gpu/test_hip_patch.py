...
def test_hip_patch_on_cuda():
    ...
    for tmp_R in matrices():
        ...
        assert cupy.allclose(tmp_G, tmp2_G, rtol=1e-11, atol=1e-11)
        back_R = irfftn_patch(tmp_G, tmp_R.shape)
        assert cupy.allclose(tmp_R, back_R, rtol=1e-11, atol=1e-11)
        back2_R = cupyx.scipy.fft.irfftn(tmp_G, tmp_R.shape)
        assert cupy.allclose(back_R, back2_R, rtol=1e-11, atol=1e-11)
