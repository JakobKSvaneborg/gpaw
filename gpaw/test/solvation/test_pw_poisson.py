...
def generate_dipole_charges(grid, pw, box):
    ...
    r_pos = np.sqrt((x - pos_center[0])**2 + (y - pos_center[1])**2 + (z - pos_center[2])**2)
    r_neg = np.sqrt((x - neg_center[0])**2 + (y - neg_center[1])**2 + (z - neg_center[2])**2)
...
def generate_quadrupole_charges(grid, pw, box, seed=None):
    """
    Generate a quadrupole-like charge distribution with four charges arranged in a tetrahedral pattern.
...
def spherical_dielectric_function(grid, box, epsinf=80.0, eps1=1.0, radius=None):
    """
    Generate a spherical dielectric function with high dielectric inside a sphere 
    and low dielectric outside, with a smooth transition at the boundary.
    ...
    radius : float, optional
        Radius of the dielectric sphere in Ångström. Defaults to 1/3 of box size.
    ...
    deps_dr_np = -(epsinf - eps1) * 0.5 * (1.0 - np.tanh((r - radius_bohr) / width)**2) / width
...
def test_cg_poisson_solver_constant_dielectric(density_generator):
    ...
    pw = PWDesc(ecut=20.0, cell=grid.cell, kpt=grid.kpt, comm=grid.comm, dtype=complex)
    ...
    solver = ConjugateGradientPoissonSolver(pw=pw, eps=1e-6, maxiter=100)
    
    class MockDielectric:
        def __init__(self, eps_gradeps):
            self.eps_gradeps = eps_gradeps
    
    solver.set_dielectric(MockDielectric(eps_gradeps))
...
def test_cg_poisson_solver_variable_dielectric(density_generator):
    ...
    eps, eps_gradeps = spherical_dielectric_function(grid, box, epsinf=80.0, eps1=1.0)
